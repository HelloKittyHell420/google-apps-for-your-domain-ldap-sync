...
class GoogleResultQueue(Queue.Queue):
  ...
  def __init__(self, **moreargs):
    super(GoogleResultQueue).__init__(self, **moreargs)
  ...
  def PutResult(self, dn, action, failure=None, object=None,
    ...
    Raises:
      Full exception, as described in Queue.put() method documentation
    """
  ...
  def GetResult(self, block=None, timeout=None):
    ...
    Return:
      tuple of (dn, action, failure, object) as described above for
        PutResult()
    Raises:
      Empty exception: as described in Queue.gett() method documentation
    """
